...
int so_MTSOS_LP(problem_params* p_params, algorithm_flags* flags, algorithm_params* a_params, optional_params* o_params, double** p_b, double** p_u, double** p_v, double* p_fx, double** p_timers, int* p_iterations){
//the primary function that performs optimization on the provided vector S
    ...
    double *S_prime, *S_dprime, dtheta, *resid, *data, *Abig_v, *bbig, *dx, *G, *indexH_v, *indexA21_v, *b_Ax, *timers, *variables;
    ...
    int i, j, status, totime, orderOkay;
    int* Abig_i, *Abig_j;
    ...
    if(S_middle==NULL || S_prime == NULL || S_dprime == NULL){
        #ifdef MATLAB_MEX_FILE
            mexErrMsgTxt("insufficient memory");
        #endif
        #ifndef MATLAB_MEX_FILE
        ...
        for(i=0;i<S_length-1;i++){
            for(j=0;j<State_size;j++){
                ...
                else if(i==S_length-2)
                    S_dprime[i*State_size+j] = (S[(i-1)*State_size+j]/2-S[i*State_size+j]+S[(i+1)*State_size+j]/2)/(dtheta*dtheta);
                else if(i == 1 || i==S_length-3)
                    S_dprime[i*State_size+j] = (S[(i-1)*State_size+j]-S[i*State_size+j]-S[(i+1)*State_size+j]+S[(i+2)*State_size+j])/(2*dtheta*dtheta);
                else
                    S_dprime[i*State_size+j] = (-S[(i-2)*State_size+j]*5/48+S[(i-1)*State_size+j]*13/16-S[i*State_size+j]*17/24-S[(i+1)*State_size+j]*17/24+S[(i+2)*State_size+j]*13/16-S[(i+3)*State_size+j]*5/48)/(dtheta*dtheta);
            }
        }
    }
    ...
    if(status){
        A21index_length = so_MakeA_LP(S, S_middle, S_prime, S_dprime, dtheta, U_size, S_length, State_size, &csA21, &indexA21_i, &indexA21_j, &indexA21_v, &b_Ax, b_0, variables, variables_length);
    ...
    while(!solved && iterations < MAX_ITERATIONS && status)
	{
        ...
        Hindex_length = so_MakeHandG_LP(b, S_middle, S_prime, S_dprime, u, a, S_length, U_size, State_size, dtheta, kappa, 0, &G, &Hc, &indexH_i, &indexH_j, &indexH_v,timers,totime,variables,variables_length);
        if(Hindex_length == -1)
            status = 0;
        else
            status = 1;
        ...
        if(!feasible && status)
		{
            ...
            if(residual < RESIDUAL_THRESHHOLD){
                ...
                fx = so_function_evaluation_LP(b, S_middle, S_prime, S_dprime, u, a, S_length, U_size, State_size, dtheta,kappa, variables,variables_length);
                if(fx == -1){
                    status = 0;
                }
            }
        ...
        if(status){
            so_stuffing_LP(Abig_v, Abig_i, Abig_j, indexH_v, indexH_i, indexH_j, Hindex_length, indexA21_v, indexA21_i, indexA21_j, A21index_length, Hc->m, S_length, State_size, U_size);
        ...
        if(status){
            ...
            orderOkay = so_lu_solve_LP(Abig_c, bbig, &lu_S, orderOkay);
            if(orderOkay==0){
                #ifdef MATLAB_MEX_FILE
        ...
        if(status){
            ...
            if(!feasible)
			{
                status = so_linesearch_infeasible_LP(&bbig[(S_length-1)*(2+U_size)], v, x, dx, b, S_middle, S_prime, S_dprime, u, a, S_length, U_size, State_size, dtheta, kappa, beta, alpha, G, csA21, b_Ax, variables, variables_length);
                if(totime)
				{//time for infeasible line search
                    diff = clock()-start;
                    time = diff*1000.0/CLOCKS_PER_SEC;
                    timers[6] += time;
                    start = clock();
			...
			else
			{
                ...
                if(status){
                    ...
                    if(Lambda/2 < epsilon){
                        ...
                        fx = so_linesearch_feasible_LP(&bbig[(S_length-1)*(2+U_size)], v, x, dx, b, S_middle, S_prime, S_dprime, u, a, S_length, U_size, State_size, dtheta, kappa, G, alpha, beta, fx, variables, variables_length);
                        if(fx == -1)
                            status = 0;
                    }
                }
...
int so_calcHbarrier_LP(double* H_barrier, int S_length, int U_size, int** pindex_i, int** pindex_j, double** pindex_v){
...
int so_MakeHandG_LP(double* b, double* S_middle, double* S_prime, double* S_dprime, double* u, double* a, int S_length, int U_size, int State_size, double dtheta, double kappa, int flag_calculate, double** G, cs** pH, int** pindexH_i, int** pindexH_j, double** pindexH_v, double* timers, int istimed, double* variables, int variables_length){
    ...
    double /**data, */*indexH_v, *indexHb_v, *H_barrier, *G_barrier, *F_barrier;
    ...
    statusB = 1;
    statusF = 0;
    index_length = S_length-1+(U_size+2)*(U_size+2)*(S_length-1);
    H_barrier = NULL;
    G_barrier = NULL;
    F_barrier = NULL;
    indexHb_i = NULL;
    ...
    if(doH && status){
        ...
        if(indexH_i == NULL || indexH_j == NULL || indexH_v == NULL){
            #ifdef MATLAB_MEX_FILE
                    mexErrMsgTxt("insufficient memory");
            #endif
            #ifndef MATLAB_MEX_FILE
            ...
            *G[0] = INFINITY;
            ...
            if(status){
                ...
                for(i=0;i<S_length-1;i++){
					...
					*(*G + i*(2 + U_size) ) = G_barrier[i*(2 + U_size)] - 1.0 - kappa / b[i + 1];
					*(*G + i*(2 + U_size) + 1) = G_barrier[i*(2 + U_size) + 1];
					*(*G + i*(2 + U_size) + 2) = G_barrier[i*(2 + U_size) + 2];
    ...
    if(doH && status){
		length_Hb = so_calcHbarrier_LP(H_barrier, S_length, U_size, &indexHb_i, &indexHb_j, &indexHb_v);
        if(length_Hb == -1)
            status = 0;
        ...
        if(status){
            memcpy(&indexH_v[S_length-1], indexHb_v, length_Hb*sizeof(double));
            memcpy(&indexH_i[S_length-1], indexHb_i, length_Hb*sizeof(int));
            memcpy(&indexH_j[S_length-1], indexHb_j, length_Hb*sizeof(int));
            
            H.nzmax = S_length-1+length_Hb;
            H.i = indexH_i;
            H.p = indexH_j;
            H.x = indexH_v;
            H.nz = H.nzmax;
...
int so_linesearch_infeasible_LP(double* w, double* v, double* x, double* dx, double* b, double* S_middle, double* S_prime, double* S_dprime, double *u, double* a, int S_length, int U_size, int State_size, double dtheta, double kappa, double beta, double alpha, double* G, cs* A21, double* b_Ax, double* variables, int variables_length) {
    ...
    if(status){
        do{
			...
			//printf("XXX\n");
            status = so_MakeHandG_LP(b, S_middle, S_prime, S_dprime, u, a, S_length, U_size, State_size, dtheta, kappa, 2, &Gnew, &Htrash, &indexH_i, &indexH_j, &indexH_v, NULL, 0, variables, variables_length);//the last three are H_i, H_j, H_v
            if(status == -1)
                status = 0;
            else
                status = 1;
...
double so_function_evaluation_LP(double* b, double* S_middle, double* S_prime, double* S_dprime, double* u, double* a, int S_length, int U_size, int State_size, double dtheta, double kappa, double* variables, int variables_length){
    ...
    if(b_okay && status){
        ...
        if(valid && status){
            for(i=0;i<S_length-1;i++){
				fx += -b[i] + F[i] - kappa*(log(b[i + 1])); /*b[i] >= 0*/
            }
        }else
            b_okay = 0;
...
int so_MakeA_LP(double* S, double* S_middle, double* S_prime, double* S_dprime, double dtheta, int U_size, int S_length, int State_size, cs** pAc, int** pA_i, int** pA_j, double** pA_v, double** pb_Ax, double b_0, double* variables, int variables_length){
    ...
    int *A_i, *A_j, *indexR_i, *indexR_j;
    int block_length, index_length;//size parameters
    int i, j, status;//indices for loops
    cs A;
    ptrdiff_t m_m,n_m,p_m;//variables for the use of mtimes;
    ...
    R_dynamics = malloc((S_length-1)*U_size*State_size*sizeof(double));
    M_dynamics = malloc((S_length-1)*State_size*State_size*sizeof(double));
    C_dynamics = malloc((S_length-1)*State_size*State_size*sizeof(double));
    d_dynamics = malloc((S_length-1)*State_size*sizeof(double));
    ...
    status = so_dynamics(S_middle, S_prime, S_length, State_size, U_size, variables, variables_length, R_dynamics, M_dynamics, C_dynamics, d_dynamics);
    ...
    if(status){
        for(i=0;i<S_length-1;i++){
            ...
            //copy M*S_prime for the matrix calculation of m.  These multiplications are performed using BLAS routines.
			cblas_dgemv(CblasRowMajor, CblasNoTrans, m_m, n_m, one, &M_dynamics[i*State_size*State_size], m_m, &S_prime[i*State_size], p_m, zero, datam, p_m);
            
            //Perform M*S_dprime
			cblas_dgemv(CblasRowMajor, CblasNoTrans, m_m, n_m, one, &M_dynamics[i*State_size*State_size], m_m, &S_dprime[i*State_size], p_m, zero, datac1, p_m);
            
            //C*S_prime^2;
			cblas_dgemv(CblasRowMajor, CblasNoTrans, m_m, n_m, one, &C_dynamics[i*State_size*State_size], m_m, &S_prime[i*State_size], p_m, zero, datac2, p_m);
            ...
            if(i==0){
                ...
                for(j=0;j<State_size;j++){
                    ...
                    A_v[i*block_length-State_size+j]= (datac1[j]+datac2[j])/2;
                    ...
                    A_v[i*block_length+State_size*(1+U_size)+2+j] = (datac1[j]+datac2[j])/2;
            ...
            for(j=0;j<State_size;j++){
                b_Ax[i*(State_size+1)+j]= d_dynamics[i*State_size+j]*-1;//the d term
                if(i==0)
                    b_Ax[i*(State_size+1)+j]-=b_0*(datac1[j]+datac2[j])/2;//the c/2*b[0] term.
            }
...
int so_lu_solve_LP(cs* A, double* b, css** S, int orderOkay) {
    double tol;
    double *x ;
    csn *N ;
    int order, n, ok ;
