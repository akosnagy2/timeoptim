...
{
    //return (CS_OVERFLOW (n,size) ? NULL : mxMalloc (CS_MAX (n,1) * size)) ;
...
{
    //return (CS_OVERFLOW (n,size) ? NULL : mxCalloc (CS_MAX (n,1), size)) ;
...
void *cs_free (void *p)
{
    //if (p) mxFree (p) ;	    /* free p if it is not already NULL */
...
void *cs_realloc (void *p, int n, size_t size, int *ok)
{
    ...
    *ok = !CS_OVERFLOW (n,size) ;	    /* guard against int overflow */
    ...
    //p2 = mxRealloc (p, CS_MAX (n,1) * size) ; /* realloc the block */
...
int cs_updown (cs *L, int sigma, const cs *C, const int *parent)
{
    ...
    w = cs_malloc ((int)n, sizeof (double)) ;
    if (!w) return (0) ;
    f = Ci [p] ;
